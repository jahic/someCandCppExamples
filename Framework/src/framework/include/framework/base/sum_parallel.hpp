#ifndef __SUM_PARALLEL_HPP__
#define __SUM_PARALLEL_HPP__
...
template<typename T>
void * sum_parallel_pthread(void * iterators__)
{
...
template<typename T>
void sum_parallel_openmp(const std::vector<T>& matrix_a_, const std::vector<T>& matrix_b_, std::vector<T>& matrix_c_, const settings & settings_ )
{
...
template<typename T>
void sum_serial(const std::vector<T>& matrix_a_, const std::vector<T>& matrix_b_, std::vector<T>& matrix_c_, const settings & settings_ )
{
...
template<typename T>
class sum_parallel : public base_parallel<T>
{
    public:

        sum_parallel() : name("sum_parallel")
        {
            
        }

        sum_parallel(const std::vector<T>& matrix_a_, const std::vector<T>& matrix_b_, std::vector<T>& matrix_c_, const settings & settings_) : name("sum_parallel")
        {
            matrix_c_.reserve(matrix_a_.size());
            this->execute(matrix_a_, matrix_b_, matrix_c_, settings_);
        }

        sum_parallel(const std::vector<T>& matrix_a_, std::vector<T>& matrix_c_, const settings & settings_) : name("sum_parallel")
        {
    ...
    private:
        ...
        void execute(const std::vector<T>& matrix_a_, const std::vector<T>& matrix_b_, std::vector<T>& matrix_c_, const settings & settings_)
        {
            ...
            {
                sum_parallel_openmp<T>(matrix_a_,matrix_b_, matrix_c_, settings_);
            }
            else 
            {
                sum_serial<T>(matrix_a_,matrix_b_, matrix_c_, settings_);
            }
        ...
        void execute_pthread(const std::vector<T>& matrix_a_, const std::vector<T>& matrix_b_, std::vector<T>& matrix_c_, const settings & settings_)
        {
            ...
            for(int i = 0; i < settings_.THREAD_NUMBER; i++)
            {
                ...
                pthread_create(threads + i, NULL, sum_parallel_pthread<T>, (void*)iterators_[i]);                  
            }
