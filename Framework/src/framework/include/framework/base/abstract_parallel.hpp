#ifndef __ABSTRACT_PARALLEL_HPP__
#define __ABSTRACT_PARALLEL_HPP__
...
template<typename T>
void * abstract_parallel_pthread(void * iterators__)
{
...
template<typename T>
void abstract_parallel_openmp(const std::vector<T>& matrix_a_, const std::vector<T>& matrix_b_, std::vector<T>& matrix_c_, const settings & settings_ )
{
	...
	for(int i = 0; i < boundary; i++)
	{
		...
		{
			matrix_c_[i * matrix_delimeter + j] = matrix_a_[i * matrix_delimeter + j] - matrix_b_[i * matrix_delimeter + j];
		}
	}
}

template<typename T>
void abstract_serial(const std::vector<T>& matrix_a_, const std::vector<T>& matrix_b_, std::vector<T>& matrix_c_, const settings & settings_ )
{
...
template<typename T>
class abstract_parallel : public base_parallel<T>
{
    public:

        abstract_parallel() : name("abstract_parallel")
        {
            
        }

        abstract_parallel(const std::vector<T>& matrix_a_, const std::vector<T>& matrix_b_, std::vector<T>& matrix_c_, const settings & settings_) : name("abstract_parallel")
        {
            matrix_c_.reserve(matrix_a_.size());
            this->execute(matrix_a_, matrix_b_, matrix_c_, settings_);
        }

        abstract_parallel(const std::vector<T>& matrix_a_, std::vector<T>& matrix_c_, const settings & settings_) : name("abstract_parallel")
        {
    ...
    private:
        ...
        void execute(const std::vector<T>& matrix_a_, const std::vector<T>& matrix_b_, std::vector<T>& matrix_c_, const settings & settings_)
        {
            ...
            {
                abstract_parallel_openmp<T>(matrix_a_,matrix_b_, matrix_c_, settings_);
            }
            else 
            {
                abstract_serial<T>(matrix_a_,matrix_b_, matrix_c_, settings_);
            }
        ...
        void execute_pthread(const std::vector<T>& matrix_a_, const std::vector<T>& matrix_b_, std::vector<T>& matrix_c_, const settings & settings_)
        {
            ...
            for(int i = 0; i < settings_.THREAD_NUMBER; i++)
            {
                ...
                pthread_create(threads + i, NULL, abstract_parallel_pthread<T>, (void*)iterators_[i]);                  
            }
