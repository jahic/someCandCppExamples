#ifndef __BLUE_CHANNEL_PARALLEL_HPP__
#define __BLUE_CHANNEL_PARALLEL_HPP__
...
template<typename T>
void * blue_channel_parallel_pthread(void * iterators__)
{
...
template<typename T>
void blue_channel_parallel_openmp(const std::vector<T>& matrix_a_, const std::vector<T>& matrix_b_, std::vector<T>& matrix_c_, const settings & settings_ )
{
	...
	for(int i = 0; i < boundary; i++)
	{
		...
		{
			matrix_c_[i * matrix_delimeter + j] = (matrix_a_[i * matrix_delimeter + j] >> 16) & 0x000000FF;
		}
	}
}
...
template<typename T>
class blue_channel_parallel : public base_parallel<T>
{
    public:

        blue_channel_parallel() : name("blue_channel_parallel")
        {
            
        }

        blue_channel_parallel(const std::vector<T>& matrix_a_, const std::vector<T>& matrix_b_, std::vector<T>& matrix_c_, const settings & settings_) : name("blue_channel_parallel")
        {
            matrix_c_.reserve(matrix_a_.size());
            this->execute(matrix_a_, matrix_b_, matrix_c_, settings_);
        }

        blue_channel_parallel(const std::vector<T>& matrix_a_, std::vector<T>& matrix_c_, const settings & settings_) : name("blue_channel_parallel")
        {
    ...
    private:
        ...
        void execute(const std::vector<T>& matrix_a_, const std::vector<T>& matrix_b_, std::vector<T>& matrix_c_, const settings & settings_)
        {
            ...
            {
                blue_channel_parallel_openmp<T>(matrix_a_,matrix_b_, matrix_c_, settings_);
            }
            else 
            {
        ...
        void execute_pthread(const std::vector<T>& matrix_a_, const std::vector<T>& matrix_b_, std::vector<T>& matrix_c_, const settings & settings_)
        {
            ...
            for(int i = 0; i < settings_.THREAD_NUMBER; i++)
            {
                ...
                pthread_create(threads + i, NULL, blue_channel_parallel_pthread<T>, (void*)iterators_[i]);                  
            }
