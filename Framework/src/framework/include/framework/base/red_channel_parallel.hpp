#ifndef __RED_CHANNEL_PARALLEL_HPP__
#define __RED_CHANNEL_PARALLEL_HPP__
...
template<typename T>
void * red_channel_parallel_pthread(void * iterators__)
{
	...
	for(int i = iterators_->begin; i < iterators_->end; i++)
	{
		...
		{
			iterators_->matrix_c[i * iterators_->width + j] = iterators_->matrix_a[i * iterators_->width + j] & 0x000000FF;
		}
	}
	return NULL;
}
...
template<typename T>
void red_channel_parallel_openmp(const std::vector<T>& matrix_a_, const std::vector<T>& matrix_b_, std::vector<T>& matrix_c_, const settings & settings_ )
{
	...
	for(int i = 0; i < boundary; i++)
	{
		...
		{
			matrix_c_[i * matrix_delimeter + j] = matrix_a_[i * matrix_delimeter + j] & 0x000000FF;
		}
	}
}

template<typename T>
void red_channel_serial(const std::vector<T>& matrix_a_, const std::vector<T>& matrix_b_, std::vector<T>& matrix_c_, const settings & settings_ )
{
...
template<typename T>
class red_channel_parallel : public base_parallel<T>
{
    public:

        red_channel_parallel() : name("red_channel_parallel")
        {
            
        }

        red_channel_parallel(const std::vector<T>& matrix_a_, const std::vector<T>& matrix_b_, std::vector<T>& matrix_c_, const settings & settings_) : name("red_channel_parallel")
        {
            matrix_c_.reserve(matrix_a_.size());
            this->execute(matrix_a_, matrix_b_, matrix_c_, settings_);
        }

        red_channel_parallel(const std::vector<T>& matrix_a_, std::vector<T>& matrix_c_, const settings & settings_) : name("red_channel_parallel")
        {
    ...
    private:
        ...
        void execute(const std::vector<T>& matrix_a_, const std::vector<T>& matrix_b_, std::vector<T>& matrix_c_, const settings & settings_)
        {
            ...
            {
                red_channel_parallel_openmp<T>(matrix_a_,matrix_b_, matrix_c_, settings_);
            }
            else 
            {
                red_channel_serial<T>(matrix_a_,matrix_b_, matrix_c_, settings_);
            }
        ...
        void execute_pthread(const std::vector<T>& matrix_a_, const std::vector<T>& matrix_b_, std::vector<T>& matrix_c_, const settings & settings_)
        {
            ...
            for(int i = 0; i < settings_.THREAD_NUMBER; i++)
            {
                ...
                pthread_create(threads + i, NULL, red_channel_parallel_pthread<T>, (void*)iterators_[i]);                  
            }
