#ifndef __GRAYSCALE_PARALLEL_HPP__
#define __GRAYSCALE_PARALLEL_HPP__
...
template<typename T>
void * grayscale_parallel_pthread(void * iterators__)
{
...
template<typename T>
void grayscale_parallel_openmp(const std::vector<T>& matrix_a_, const std::vector<T>& matrix_b_, std::vector<T>& matrix_c_, const settings & settings_ )
{
	...
	for(int i = 0; i < boundary; i++)
	{
		...
		{
			matrix_c_[i * matrix_delimeter + j] = ( ((matrix_a_[i * matrix_delimeter + j])&0x000000FF) + ((matrix_a_[i * matrix_delimeter + j] >> 8)&0x000000FF) + ((matrix_a_[i * matrix_delimeter + j] >> 16)&0x000000FF) ) / 3;
		}
	}
}
...
template<typename T>
class grayscale_parallel : public base_parallel<T>
{
    public:

        grayscale_parallel() : name("grayscale_parallel")
        {
            
        }

        grayscale_parallel(const std::vector<T>& matrix_a_, const std::vector<T>& matrix_b_, std::vector<T>& matrix_c_, const settings & settings_) : name("grayscale_parallel")
        {
            matrix_c_.reserve(matrix_a_.size());
            this->execute(matrix_a_, matrix_b_, matrix_c_, settings_);
        }

        grayscale_parallel(const std::vector<T>& matrix_a_, std::vector<T>& matrix_c_, const settings & settings_) : name("grayscale_parallel")
        {
    ...
    private:
        ...
        void execute(const std::vector<T>& matrix_a_, const std::vector<T>& matrix_b_, std::vector<T>& matrix_c_, const settings & settings_)
        {
            ...
            {
                grayscale_parallel_openmp<T>(matrix_a_,matrix_b_, matrix_c_, settings_);
            }
            else 
            {
        ...
        void execute_pthread(const std::vector<T>& matrix_a_, const std::vector<T>& matrix_b_, std::vector<T>& matrix_c_, const settings & settings_)
        {
            ...
            for(int i = 0; i < settings_.THREAD_NUMBER; i++)
            {
                ...
                pthread_create(threads + i, NULL, grayscale_parallel_pthread<T>, (void*)iterators_[i]);                  
            }
