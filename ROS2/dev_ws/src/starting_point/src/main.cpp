...
class MinimalPublisher : public rclcpp::Node
{
  ...
  private:
    void timer_callback()
    {
        ...
        {
            rclcpp::allocator::allocator_stack<int, 4096> allocator_stack;
            for(int j = 0; j < 4096; j++)
            {
                allocator_stack.push_back(j);
            }
        }
        ...
        message.data = std::to_string(std::chrono::duration_cast<std::chrono::milliseconds>(after - before).count());
...
private:
  ...
  //rclcpp::create_publisher<std_msgs::msg::String> publisher = 
